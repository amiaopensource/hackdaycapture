...
if [ "${video_codec}" = "uncompressed" ] ; then
    if [ "${pixel}" = "10" ] ; then
...
/usr/local/bin/bmdcapture -v -m 0 -V "${video_input}" -A 1 -c 8 -s 32 -p "${pixel}" -F nut -f pipe:1 | \
     /usr/local/bin/ffmpeg -y -v info -hide_banner -stats -i - ${middleoptions[@]} \
        ...
        -map "[stereo1]" \
        -map "[stereo2]" \
        "${output_dir}/${output_file}.${container}" \
        -c copy -f nut - | \
        /usr/local/bin/ffplay -v info -hide_banner -stats - \
